...
const dropModalTemplate = status => `
    <div
      id="modalBackground"
...
const noDataTemplate = status => `
  <div class="m-auto text-center py-7 text-stone-500 dark:text-slate-300">${NO_DATA_TEXT[status]}</div>
`;

const template = data => `<li data-id="${data.id}" class="px-3 relative">
    <div class="view relative flex">
    <input id="toggle" class="toggle checked:border w-8 border-none appearance-none absolute inset-y-0 cursor-pointer peer/input" type="checkbox" ${
